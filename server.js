...
app.post("/api/generate-titles", async (req, res) => {
  try {
    ...
    const prompt = `あなたはSEO検定１級合格者の専門家です。以下のキーワードに対して、検索上位を狙える魅力的なタイトルを5つ生成してください。
...
app.post("/api/generate-article", async (req, res) => {
  try {
    ...
    const prompt = `あなたはseo検定１級合格者で専門性の高いプロのSEOライターです。Google検索で上位表示されるために、E-A-T（専門性・権威性・信頼性）を重視した高品質な記事を生成してください。
